...
**Status:** v1 complete and deployed — live on Streamlit Community Cloud, with a separate staging environment for testing changes before they reach production (see [`docs/staging-vs-production.md`](docs/staging-vs-production.md)). Independently evaluated (see [`docs/rag-evaluation.md`](docs/rag-evaluation.md)), and both real bugs that evaluation surfaced — messy PDF text reaching the chunker, and chunks split mid-sentence — have since been fixed and covered by an automated test suite.
...
- Storage is local-only (SQLite file + Chroma directory on disk) and ephemeral on Streamlit Community Cloud — a reboot or redeploy wipes uploaded papers. Deferred deliberately for a portfolio/demo app; see `CLAUDE.md` for the migration path (hosted Postgres + Chroma Cloud, or a paid host with a persistent volume) if this becomes something used across days/weeks.
- No automated answer-quality (LLM-as-judge) checking — only technical health (errors, response time, "I don't know" rate) is monitored; see [`docs/safety-monitoring.md`](docs/safety-monitoring.md).
...
- [`docs/staging-vs-production.md`](docs/staging-vs-production.md) — the two-environment deploy setup and when a change goes through staging first
- [`docs/deployment-rationale.md`](docs/deployment-rationale.md) — why Streamlit Community Cloud, and the tradeoffs accepted to get there
