...
| 4.22 | External Integration | **RAG for document Q&A** | `rag-evaluation.md`, 10+ docs, 5 working + 3 failing queries documented | **Unblocked, ready to write (2026-08-04)** — capability complete and verified since 2026-08-01. The workspace (`abeer-test`) now has **10 real papers** (guidelines, clinical management, 2 diagnostics papers, 2 vaccine papers, Pakistan epidemiology + genomics) — the trigger condition for writing the eval is met. **Next action for a future session: write `docs/rag-evaluation.md` for real** — run 10+ real questions against this corpus, document 5 that work well and 3 that fail/fall short with why, and deliberately include at least one broad multi-paper-relevant question to check whether `n_results=5` lets 1-2 papers crowd out others (flagged 2026-08-03; not yet tested against a real multi-topic corpus). This is the centerpiece SLO of the whole project — the only remaining piece. |
