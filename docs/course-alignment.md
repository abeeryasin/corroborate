...
| 5.6 | Autonomous Operations | Escalation protocol | `escalation-protocol.md` | **Satisfied (2026-08-21)** — `docs/escalation-protocol.md`, written by the user after a 2-question comprehension quiz on the freshly-explained concept (no feature built — genuinely didn't need one to demonstrate the idea). Correctly distinguishes Corroborate's current "I don't know" stop (informs the end user) from a true escalation protocol (alerts a human operator on defined thresholds — error rate, spend, "I don't know" spikes). Deferred the real implementation to v3 as a deliberate, explained scope call, not an oversight |
...
| 5.13 | Autonomous Operations | Production infra / SLA mindset | `sla-definition.md`, 2wk uptime log | **Satisfied (2026-08-22)** — `docs/sla-definition.md`, written by the user after a 2-question quiz, no corrections needed either time. A real `session_log` table + logging was actually built, then deliberately reverted before pushing once the user directly questioned it: the data could only ever prove uptime, never downtime, and with a single user who *is* the operator, there's no one to make an availability promise to that they don't already know firsthand. 2-week uptime log requirement adapted: states Corroborate's actual honest SLA instead of fabricating data that can't yet exist |
| 5.14 | Autonomous Operations | Staging vs. production | Working staging env | **Satisfied (2026-08-23)** — a genuinely working staging environment: a `staging` git branch + a second Streamlit Community Cloud app tracking it, separate from production. `docs/staging-vs-production.md` written by the user after a 3-question quiz, no corrections needed. Adopted going forward for real code changes; docs-only edits still go straight to `main`. Flagged too late — should have been raised before the first deploy, not after several pushes (including a real incident) had already gone straight to production; the honest process-miss and the broader lesson (some SLOs are prospective, not retrospective) are logged in `docs/decisions.md` (2026-08-23) |
| 5.15 | Autonomous Operations | Observability | Dashboard, cost/latency/startup | **Satisfied (2026-08-23)** — `docs/observability.md`, written by the user after a 1-question quiz (comprehensive enough to cover all three parameters unprompted, one minor sharpening on the startup-attribution point). Real cost tracking added to the System health dashboard using actual Claude Opus 4.8 pricing computed from `response.usage`; latency already covered by SLO 5.5; startup left honestly unmeasured for the same reason uptime tracking was reverted — can't attribute a slow response to a cold start specifically without more infrastructure than this scale justifies |
