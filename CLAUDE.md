...
1. Once picked back up, go through **every SLO in `docs/course-alignment.md` except the 5 marked Skip (team-dependent)** — not just a review of this project's own decisions. Satisfied SLOs need a genuine understanding-check (quiz-style); the ~33 "Not started" ones need real concept-teaching first, since they haven't been covered at all yet. Framed around interview-readiness (one of the 5 original success criteria) — explicitly requested by the user, not yet done, and bigger in scope than a single session.
